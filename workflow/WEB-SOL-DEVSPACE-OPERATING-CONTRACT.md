# Web GPT-5.6 Sol XHIGH + DevSpace 검증 운영 계약
...
- 로그인·계정 전역 설정 실패와 Oracle 소유 임시채팅 맞춤화 실패를 구분합니다.
- Oracle 소유 임시채팅의 맞춤화는 검증된 helper가 자동 처리하며, 매 run 사용자 수동 토글을 기본 절차로 만들지 않습니다.
...
**한 줄 원칙: Web Sol XHIGH는 실제 파일을 읽고 만들고 기존 파일을 정밀 수정하는 주력 실무자로 사용할 수 있다. 계정 전역 개인화와 Oracle 소유 임시채팅 맞춤화를 구분하고, 삭제·이동·이름 변경은 기본 금지하며, Sol 관리자가 diff와 테스트를 독립 검증한 뒤 통합한다.**
